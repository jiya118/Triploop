...
const ItineraryBuilder = () => {
  // Aligned with Table 4: trip_stops and Table 6: stop_activities
  const [stops, setStops] = useState([
    {
      ...
      activities: [
        { id: 'act-1', name: 'Eiffel Tower Visit', time: '10:00 AM', cost: 28 }
      ]
    }
  ]);
        ...
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div>
            <nav className="flex gap-2 text-xs font-bold text-blue-600 uppercase tracking-widest mb-2">
              <span>My Trips</span> <ChevronRight size={12}/> <span>Europe Summer</span>
            </nav>
              ...
              <div className={cardStyle}>
                <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-6">
                  ...
                  <div className="flex gap-2">
                    <button className="p-3 bg-slate-50 text-slate-400 hover:text-red-500 rounded-xl transition">
                      <Trash2 size={18}/>
                    </button>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {stop.activities.map((act) => (
                    <div key={act.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 group">
                      ...
                      <span className="font-black text-blue-600">${act.cost}</span>
                    </div>
                  ))}
                  <button className="w-full py-3 border-2 border-dashed border-slate-200 rounded-2xl text-sm font-bold text-slate-400 hover:border-blue-300 hover:text-blue-500 hover:bg-blue-50 transition-all flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="flex gap-3">
              ...
              <button className="flex-2 py-4 px-8 bg-blue-600 text-white font-black rounded-2xl shadow-lg shadow-blue-100 transition transform hover:-translate-y-1">Confirm Stop</button>
            </div>
          </div>
        </div>
      )}
