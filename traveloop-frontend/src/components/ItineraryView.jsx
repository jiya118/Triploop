...
import { Calendar, MapPin, Clock, DollarSign, Share2, Download, ChevronLeft, LayoutGrid, List } from 'lucide-react';
    ...
    stops: [
      {
        ...
        dates: "Jun 20 - Jun 23",
        activities: [
          { name: "Eiffel Tower", time: "10:00 AM", cost: 28, type: "Sightseeing" },
          { name: "Seine River Cruise", time: "08:00 PM", cost: 20, type: "Sightseeing" }
        ]
      },
      {
        city: "Tokyo",
        dates: "Jun 25 - Jun 28",
        activities: [
          { name: "Tsukiji Food Tour", time: "06:00 AM", cost: 35, type: "Food" },
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900">
      ...
      <div className="sticky top-0 bg-white/90 backdrop-blur-md border-b border-slate-100 z-50 px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button className="p-2 text-slate-600 hover:bg-slate-100 rounded-xl transition"><Share2 size={20}/></button>
          <button className="p-2 text-slate-600 hover:bg-slate-100 rounded-xl transition"><Download size={20}/></button>
        </div>
      </div>
        ...
        <div className="mb-12 text-center md:text-left">
          ...
          <div className="flex flex-wrap gap-6 justify-center md:justify-start items-center">
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-green-50 text-green-700 rounded-2xl font-bold text-sm">
              <DollarSign size={16} /> Est. ${itineraryData.totalBudget}
            </div>
          </div>
        </div>
                ...
                {stop.activities.map((act, aIdx) => (
                  <div key={aIdx} className="p-6 rounded-[2rem] border border-slate-100 bg-slate-50/50 hover:bg-white hover:shadow-xl transition-all duration-500 group">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <span className="text-sm font-black text-slate-900">${act.cost}</span>
                    </div>
